...
class DispatcherCommand extends Command {
    ...
    execute(next) {
        ...
        if (isLocaleSpecificMetadataRequest) {
            const cachedMetadata = this._metadataCache.get(contract.getContentTypeInfo().getMimeType(), locale);
            if (cachedMetadata) {
                this._logger.info('Skip dispatching, as $metadata was retrieved from cache');
        ...
        this._logger.info('Start dispatching...');
        ...
        this._dispatcher.dispatch(this._request, this._response, this._componentManager)
            .then(result => {
                if (isLocaleSpecificMetadataRequest && result.data != null && result.data.value != null) {
                    ...
                    const metadataEtag = this._metadataCache.get(contract.getContentTypeInfo().getMimeType(), locale)
                        .etag;
