const builtin = {
  get types () {
    ...
    Object.defineProperty(builtin, 'types', {value: types})
    return types
  }
}

const _buildColumn = (name, type, value, key, annotations) => {
  const column = Object.assign({name: name, type: type, value: value}, annotations)
  if (key) {
    column.key = true
  }
  return column
...
const _getFinalType = (definitions, type, parentAnnotations) => {
  if (builtin.types[type] || !definitions[type]) {
...
const getColumns = (reflected, entity, prefix, parentAnnotations = {}) => {
  ...
  for (const key of Object.keys(entity.elements)) {
    ...
    if (type !== 'cds.Association' && builtin.types[type]) {
      // simple column
      columns.push(_buildColumn(name, type, element.value, element.key, annotations))
    } else if (type === 'cds.Association' && element.foreignKeys) {
      // managed association
      Object.keys(element.foreignKeys).forEach((key) => {
        columns.push(_buildColumn(`${name}_${key}`, reflected.definitions[element.target].elements[key].type, undefined, element.key, annotations))
      })
