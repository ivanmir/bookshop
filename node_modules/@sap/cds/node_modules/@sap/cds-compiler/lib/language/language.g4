...
@after { $source.namespace = this.attachLocation($decl); }
  ...
  |
    ...
    { this.message( 'syntax-obsolete-keyword', $prefix,
                    { obsolete: 'NAMEPREFIX', recommended: 'NAMESPACE' },
                    'Warning', 'Keyword $(OBSOLETE) is obsolete, use $(RECOMMENDED) instead' ); }
  ;
...
@after { /* #ATN 2 */ this.attachLocation($art); }
  :
    ( CONTEXT | abs=ABSTRACT? service=SERVICE ) simplePath[ $name ]
...
@after { this.attachLocation($art); }
  :
    ...
    parameterListDef[ $art ]? // WITH PARAMETERS
    (
      ( ':'
        includeRef[ $art ]
        ( ',' { if (this.isStraightBefore('{')) break; } // allow ',' before '{'
...
@after { this.attachLocation($art); }
  :
    ...
    { this.message( null, $expected, 'EXTEND PROJECTION is not supported' );
      $art = this.addItem( $outer, 'extensions', 'extend', $annos,
  ...
  :
    ...
    (
      ...
      // path { x as x1 } as p1
      {
        ...
        else if ($e.expr && !this.options.parseOnly)
          // FIXME: move this into definer.js (if we produce `columns`...)
          this.message( null, $e.expr.location, 'Required alias name for select item' );
      }
      (
...
viewParameterDef[ outer ] locals[ art, annos = [] ]
@after { this.attachLocation($art); }
  :
    annotationAssignment_ll1[ $annos ]*
    inMode=IN?
...
@after { this.attachLocation($art); }
  :
    ...
    (
      WITH PARAMETERS
      viewParameterDef[ $art ]
      ( ',' viewParameterDef[ $art ] )* // no optional final ',' here
    )?
...
typeToOne[ art ]
  :
    one=ONE
    { $art.cardinality = { targetMax: this.numberLiteral( $one, null, '1' ),
...
typeToMany[ art ]
  :
    many=MANY
    { $art.cardinality = { targetMax: { literal: 'string', val: '*',
...
@after { this.attachLocation($query); }
  ...
  |
    ...
    |
      // epsilon
      {
        $query.all = { val: 'implicit',
                       location: $t1.query && $t1.query.location || this.tokenLocation( $select ) };
        $query.elements = Object.create(null);
      }
    )
    (
...
@after{ this.attachLocation($qp); }
  :
    ( dc=':' {
      ...
      this.hanaFlavorOnly( 'Scope operator ":" in expressions is not supported yet', $dc ); }
...
@after{ /* #ATN 1 */ }
  :
    ...
    |
      { this.hanaFlavorOnly( 'Function arguments with \'=>\' are not supported yet', $paren );
        $pathStep.args = []; }
    ...
    |
      star='*'
      { this.hanaFlavorOnly( '\'*\' in functions is not supported yet', $star );
        $pathStep.args = []; }
    |
      { $pathStep.args = []; }
    )
...
simplePath[ art, attachScope = false ] locals[ _sync = 'nop' ]
@after { this.attachLocation($art); }
