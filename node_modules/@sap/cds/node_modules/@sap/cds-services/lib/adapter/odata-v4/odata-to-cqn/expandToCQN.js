...
const {QueryOptions, edm: {EdmTypeKind}} = require('@sap/odata-v4')
...
const _getColumnsFromTargetType = (targetType, all = false) => {
  ...
  if (all) {
    return Array.from(targetType.getProperties()).map((element) => ({ref: [element[0]]}))
  }

  return Array.from(targetType.getKeyPropertyRefs().keys()).map((element) => ({ref: [element]}))
}
...
const _getSelectedElements = (expandItem, targetType) => {
  ...
  for (const selectItem of innerSelectItems) {
    for (const segment of selectItem.getPathSegments()) {
      if (!selectedPaths.some(({ref: [ref]}) => { return (ref === segment.getPathSegmentIdentifier()) })) {
...
const _filter = (item, expression) => {
  ...
  item.where = ql.statements.SELECT.from('a').where(filterToCQN(expression, expression.getOperator())).SELECT.where
}
...
const _getItemCQN = (maxExpandSize, name, navigationProperty, expandItem) => {
  ...
  _limit(item, expandItem.getOption(QueryOptions.TOP) || maxExpandSize, expandItem.getOption(QueryOptions.SKIP) || 0, maxExpandSize)
