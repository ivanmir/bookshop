const {
  ...
  uri: { UriResource: { ResourceKind: { COUNT, NAVIGATION_TO_ONE } } }
...
const _getCount = (service, readContext, shareContext = false) => {
  ...
  const select = readContext.statements.SELECT.from(
    readContext.query.SELECT.from,
    [{ 'COUNT(1)': 'counted' }]
  )
...
const _readEntityOrProperty = (service, context, segments) => {
  return service.processEvent('READ', context).then(result => {
    ...
    return toODataResult(
      propertyElement === null
        ? result[0]
        : result[0][propertyElement.getName()]
    )
  })
}
...
const _readCollectionNoCount = (service, context, req) => {
  return service.processEvent('READ', context).then(result => {
    ...
    if (pageSize && result.length === pageSize) {
      modifiedResult['*@odata.nextLink'] =
        skipToken(req.getUriInfo()) + pageSize
    }
...
const _readCollectionWithCount = (service, context, req) => {
  ...
  if (context.query.SELECT.limit) {
    ...
    return Promise.all([
      _readCollectionNoCount(service, context, req),
      _getCount(service, context, true)
    ]).then(([result, count]) => {
      ...
      if (finishTransaction && context._.endTransactionIfConnected) {
        ...
        return context._
          .endTransactionIfConnected(context, 'rollback')
          .then(() => {
            return result
          })
      }
