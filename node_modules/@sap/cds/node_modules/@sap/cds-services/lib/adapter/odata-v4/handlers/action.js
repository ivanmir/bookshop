const { Components: { ACTION_EXECUTE_HANDLER } } = require('@sap/odata-v4')
...
const _getEventName = (service, context, req) => {
  ...
  return `${service.service}.${lastSegment}`
}
...
const _getActionReturnType = (service, target, actionName) => {
  const returns = _getTypeReturns(
    service.definition.definitions,
    target,
    actionName
  )
    ...
    service
      ...
      .then(result => {
        next(
          null,
          _convertResult(
            _getActionReturnType(service, context.target, actionName),
            result
          )
        )
      })
