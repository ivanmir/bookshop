...
const _givenOptions = (options, init) => {
  if (init) {
    return options || {}
  }
...
const _options = (options, init) => {
  options = _givenOptions(options, init)
...
function service (csn, options, init) {
  ...
  const serviceOptions = _options(options, init)
  ...
  return new Service(csn, serviceOptions, _getImplementation(serviceDefinition, options, init))
}
